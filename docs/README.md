...
1. Se necessario, aggiornare le variabili nei vari `ansible/inventories/`

2. Salvare la passphrease all'interno del file `.vault_pass.txt`
...
4. Lanciare il comando Ansible per il deployment dipendentemente dall'infrastruttura di deployment scelta:
   ...
   - Test ( GARR Cloud Catania ):
     ...
     ansible-playbook ansible/playbook.yml -i ansible/inventories/ct1/inventory.ini --vault-password-file .vault_pass.txt -u GARR_USER
     ```
   ...
   - Produzione in caso di Disaster-Recovery ( GARR Cloud Palermo ):
     ...
     ansible-playbook ansible/playbook.yml -i ansible/inventories/pa1/inventory.ini --vault-password-file .vault_pass.txt -u GARR_USER
     ```
   ...
   - Produzione ( GARR INFRA Bari ):
     ...
     ansible-playbook ansible/playbook.yml -i ansible/inventories/ba1/inventory.ini --vault-password-file .vault_pass.txt -u GARR_USER
     ```
...
02. Creare `/opt/idem-certbot/docker-compose.yml`:
    ...
    services:
    idem-certbot:
        image: "gitlab.dir.garr.it:4567/idem/idem-certbot:{{ tag }}"
...
03. Creare `/opt/idem-certbot/.certbot.env`:
...
06. Una volta avviato il container è possibile eliminare il file `.certbot.env`.
